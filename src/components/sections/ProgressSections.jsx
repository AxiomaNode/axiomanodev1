...
// ── Discipline + gap analysis helpers ────────────────────────────────────────
...
// Compute gaps closed this month:
// A gap is "closed" if it appeared in at least one session this month
// but is NOT present in the most recent session.
const computeGapsClosedThisMonth = (diagnostics) => {
  ...
  // Gaps seen at any point this month
  ...
  // Gaps in the most recent session
  const latest = thisMonth[thisMonth.length - 1];
  const latestGapKeys = new Set((latest.gaps || []).map(g => g.coreGapId || g.id));

  // Closed = seen before but absent in latest
  let closed = 0;
  seenThisMonth.forEach(key => {
    if (!latestGapKeys.has(key)) closed++;
  });
  return closed;
};

// Compute discipline insights — returns array of { icon, text, positive }
const computeInsights = (diagnostics, practice) => {
  ...
  // Improvement trend — last 3 vs prior 3 diagnostics
  ...
  if (sorted.length >= 6) {
    ...
    const recent = avg(sorted.slice(0,3));
    const prior  = avg(sorted.slice(3,6));
    const delta  = Math.round(recent - prior);
    if (delta >= 5) {
      insights.push({ type: "up", text: `Accuracy up ${delta}% over your last 3 diagnostics.`, positive: true });
    } else if (delta <= -5) {
      insights.push({ type: "down", text: `Accuracy down ${Math.abs(delta)}% over your last 3 diagnostics. Review gaps.`, positive: false });
    }
  }
  ...
  if (activeDays >= 10) {
    insights.push({ type: "calendar", text: `Active ${activeDays} days this month.`, positive: true });
  } else if (activeDays >= 5) {
    insights.push({ type: "calendar", text: `Active ${activeDays} days this month — keep building the habit.`, positive: null });
  }

  // Gap closed signal
  const gapsClosed = computeGapsClosedThisMonth(diagnostics);
  if (gapsClosed > 0) {
    insights.push({ type: "trophy", text: `${gapsClosed} reasoning gap${gapsClosed !== 1 ? "s" : ""} closed this month.`, positive: true });
  }

  return insights.slice(0, 4); // cap at 4
};

// ── Discipline Insights Block ─────────────────────────────────────────────────
...
const DisciplineInsights = ({ diagnostics, practice }) => {
  const insights = useMemo(
    () => computeInsights(diagnostics, practice),
    ...
    <div className="progress-insights">
      ...
      <div className="progress-insights__list">
        {insights.map((ins, i) => (
          <div
            key={i}
            className={`progress-insight progress-insight--${ins.positive === true ? "good" : ins.positive === false ? "warn" : "neutral"}`}
          >
            <span className="progress-insight__icon">
...
const ProgressChart = ({ diagnostics, practice }) => {
  ...
  const lineStyle = (delay = 0) => ({
    ...
    transition: drawn ? `stroke-dashoffset 1.4s cubic-bezier(0.4,0,0.2,1) ${delay}ms` : "none",
  });
...
const ProgressSection = ({ diagnostics = [], practice = [], onDiagnosticClick }) => {
  ...
  // Item 17 — gaps closed this month
  const gapsClosedThisMonth = useMemo(
    () => computeGapsClosedThisMonth(diagnostics),
    [diagnostics]
  );
  ...
  return (
    <div className="progress-section-inner">
      ...
      {/* Stats row — includes gaps closed this month (item 17) */}
