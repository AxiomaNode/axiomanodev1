...
/**
 * Resolve an answer key (e.g. "B") to its full display text.
 *
 * buildOptions() in questionTemplates.js produces:
 *   options: [{ value: "Full answer text", label: "A" }, ...]
 *   correct: "A"   ← the label letter
 *
 * So we find the option whose .label matches the key and return .value.
 * Also handles legacy shapes just in case.
 */
const resolveAnswerText = (q, key) => {
  if (!key) return null;
  const opts = q?.options;
  if (!opts) return key;

  // Current shape from buildOptions: [{ value: "text", label: "A" }, ...]
  if (Array.isArray(opts) && opts.length && typeof opts[0] === "object") {
    const found = opts.find((o) => o.label === key || o.id === key);
    // .value holds the human-readable text; .label is just the letter
    return found?.value ?? found?.text ?? key;
  }

  // Legacy: array of plain strings — A=index 0, B=index 1, …
  if (Array.isArray(opts) && typeof opts[0] === "string") {
    const idx = key.charCodeAt(0) - 65;
    return opts[idx] ?? key;
  }

  // Legacy: object map { A: "text", B: "text", … }
  if (!Array.isArray(opts)) return opts[key] ?? key;
              ...
              <div className="results-breakdown__item-answers">
                <div className="results-breakdown__answer results-breakdown__answer--wrong">
                  ...
                  <span className="results-breakdown__answer-value">
                    {isSkipped ? "— skipped" : userAnswerText}
                  </span>
                </div>
                <div className="results-breakdown__answer results-breakdown__answer--correct">
                  ...
                  <span className="results-breakdown__answer-value">{correctAnswerText}</span>
                </div>
              </div>
            </div>
          );
